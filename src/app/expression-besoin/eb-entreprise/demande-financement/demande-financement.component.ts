...
export class DemandeFinancementComponent implements OnInit, OnChanges {
  date: any;
  ...
  demandeFinancementForm = this.fb.group({
    ...
    garantie: [''],
    valeurGarantie: ['', [Validators.pattern('^[0-9\\s]*$')]],
 ...
 /* gestionGrilleMontant(){
  ...
  /*controlLabelGarante(){
    if(JSON.parse(this.demandeFinancementForm.controls['garantie'].value).libelle == 'Sans garantie' ){
      this.demandeFinancementForm.controls["valeurGarantie"].setValue('');
      this.demandeFinancementForm.controls["valeurGarantie"].disable();
    }
    else{
      this.demandeFinancementForm.controls["valeurGarantie"].enable();
    }
  }*/
