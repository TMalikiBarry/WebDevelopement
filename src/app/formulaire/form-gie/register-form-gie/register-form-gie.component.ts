...
@Component({
  selector: 'app-register-form-gie',
  templateUrl: './register-form-gie.component.html',
  styleUrls: ['./register-form-gie.component.scss']
})
export class RegisterFormGieComponent implements OnInit, OnChanges {
  ...
  @Input() benefApiResponse : ApiResponseBenef = new ApiResponseBenef()  ;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if(this.idPmo && this.showSuccessPmo){
      this.router.navigate(['/expression-besoin/expression-gie/'+this.idPmo+'/'+this.benefApiResponse?.reponse?.idBeneficiaire]);
    }
  }
  ...
  handleOkStep(): void {
    if(this.stepname === "la personne contact"){
      this.formGieComponent.currentStepPosition =0;

    }else if(this.stepname === "l'agent"){
      this.formGieComponent.currentStepPosition =1;

    }else if(this.stepname === "groupement"){
      this.formGieComponent.currentStepPosition =2;

    }else if(this.stepname === "l'activité de l'entreprise"){
      this.formGieComponent.currentStepPosition =3;

    }else if(this.stepname === "la pièce d'identite recto et verso"){
      this.formGieComponent.currentStepPosition =4;
