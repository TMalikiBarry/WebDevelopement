...
@Component({
  selector: 'app-register-form-entreprise',
  templateUrl: './register-form-entreprise.component.html',
  styleUrls: ['./register-form-entreprise.component.scss']
})
export class RegisterFormEntrepriseComponent implements OnInit, OnChanges {


  @Input() benefApiResponse: any;
  ...
  connexionForm = this.fb.group({
    username: ['', [Validators.required,
      ...
      this.loginChecker.checkLoginExist()
    ],
  ...
  ngOnChanges(changes: SimpleChanges): void {
    // console.log(changes.showSuccessPmo);
    if (this.idPmo && this.showSuccessPmo) {
      this.router.navigate(['/expression-besoin/expression-pme/' + this.idPmo + '/' + this.benefApiResponse?.reponse?.idBeneficiaire]);
    }
  }
  ...
  handleOkModal(): void {
    ...
    this.otpService.sendCodeNewNumero(newTelephone, this.benefApiResponse.reponse?.idPersonne).subscribe(
      // this.otpService.regenerateCode(newTelephone , 89).subscribe(
      response => {
    ...
    if (this.otpForm.valid) {
      this.otpService.sendCode(this.otpForm.controls['code'].value, this.benefApiResponse.reponse?.idPersonne).subscribe(
        ...
        error => {
          this.isSpinningRegister = false;
          this.errorOtp();
          //this.notification.error('Attention','Une erreur est survenue, valider le code ')
        }
      )
    }
  ...
  handleOkStep(): void {
    if (this.stepname === "la personne contact") {
      this.formEntrepriseComponent.currentStepPosition = 0;

    } else if (this.stepname === "l'agent") {
      this.formEntrepriseComponent.currentStepPosition = 1;

    } else if (this.stepname === "l'entreprise") {
      this.formEntrepriseComponent.currentStepPosition = 2;

    } else if (this.stepname === "dirigeants et assoscies") {
      this.formEntrepriseComponent.currentStepPosition = 3;

    } else if (this.stepname === "l'activité de l'entreprise") {
      this.formEntrepriseComponent.currentStepPosition = 4;

    } else if (this.stepname === "la pièce d'identite recto et verso") {
      this.formEntrepriseComponent.currentStepPosition = 5;
  ...
  openCgu(): void {
    this.modal.info({
      ...
      nzContent: '<div xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html" xmlns="http://www.w3.org/1999/html">' +
        '<hr></hr>' +
        '<p>Les présentes Conditions Générales d’Utilisation du site Suqali.com sont conclues entre :\n' +
        '\n' +
        ...
        ' - Adresse URL du site est : <a routerLink="https://suqali.com">https://suqali.com</a></br>' +
        ' - Email : info@suqali.com</br>' +
        ...
        '<p>Les présentes Conditions Générales d’Utilisation sont conclues pour une durée indéterminée et produisent des effets à l’égard des Utilisateurs à compter de leur adhésion en vue de l’utilisation des services proposés. Le site https://suqali.com/ se réserve le droit de modifier unilatéralement les clauses des présentes Conditions Générales d’Utilisation à tout moment et sans justification. Ces modifications s’appliqueront ipso facto à leur date de publication sur le site.</p>' +
