...
@Component({
  selector: 'app-pme-selphie',
  templateUrl: './pme-selphie.component.html',
  styleUrls: ['./pme-selphie.component.scss']
})
export class PmeSelphieComponent implements OnInit {
  beneficiaireId: any;
...
// img variables
  ...
  imageRecto: any;
  imageVerso: any;
  currentStepPosition: number = this.formEntreprise.currentStepPosition;
...
// switch to next / previous / specific webcam; true/false: forward/backwards, string: deviceId
  ...
  constructor(private fb: UntypedFormBuilder,
              ...
              private formEntreprise: FormEntrepriseComponent,
...
//console.log(value)
  ...
  beforeUploadSelfie = (file: NzUploadFile): boolean => {
    ...
    this.getBase64(this.fileSelfie)
      .then(
        (value)=>{
          //console.log(value)
...
// temporary
    if (this.nomDocumentCNIRecto && this.nomDocumentSelphie && this.nomDocumentCNIVerso) {
      ...
      if ((this.auth.getRole() == 'AGENT_INITIATEUR')) {
          this.formEntreprise.sendEntrepriseInfoUpdate();
        return;
      }
...
// face rekognition environnement variables check before handling
      if (environment.faceRekognition) {
        this.verifyValidate.verifyValidate(formData).subscribe(
          response => {
            //console.log('success face ' , response );
            this.isSpinning = false;
            this.changeDetectorRef.markForCheck();

            this.checkFaceOk = true;

            //  register now the file on suqali
