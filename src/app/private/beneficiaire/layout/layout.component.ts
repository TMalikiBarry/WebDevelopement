...
export class LayoutComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('currentUser')) {
      ...
      this.onGetBeneficiaire(this.user.idParent)

      // CHECK IF THE USER CAN DO DEMANDE
      this.demandeService.checkDemandeValidity(this.user.idParent).subscribe(
        response => {
  ...
  onGetAllOffres(){
    ...
    this.beneficiaireService.getAllOffres(this.idBeneficiaire).subscribe((response) => {
      ...
      if(response.demande!=null && response.offres!=null){
        ...
        if(this.demandeSelection.demande?.statutDossier?.trim() == 'INITIE' || this.demandeSelection.demande?.statutDossier?.trim() == 'RECU_AF'
        ...
        else if(this.demandeSelection.demande?.statutDossier?.trim() == 'RECU'){
        ...
        if(this.listeOffres.length==0 || this.statusFinancementNumber > 1 || this.demandeSelection.demande?.statutDossier?.trim() != 'MATCHE'){
